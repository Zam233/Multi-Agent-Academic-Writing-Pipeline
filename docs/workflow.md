...
2. 中外文献检索（中文库一次一关键词；外文库；必要时 web 补充前沿）——对每篇候选文献取回**完整条目并落盘** `_文献池_最新.md`（完整摘要+全部元数据，不截断；接口未提供字段才注明"接口未提供"）；
3. **文献传递协议**：委派 planner/writer/auditor 时必须让其读到含摘要的完整条目（首选读 `_文献池_最新.md`，否则内联）；**严禁只传"作者+题名+出处"精简清单**——writer 无摘要不得引文，auditor 以摘要核对观点归属；
4. 产出写作规划报告：核心问题与论证目标 / 段落结构（细化到分论点+篇幅分配）/ 拟引文献清单及**每篇的具体用途（依摘要判定）** / 与论文底层画像的咬合点 / 写作难点与需作者补充的信息；
